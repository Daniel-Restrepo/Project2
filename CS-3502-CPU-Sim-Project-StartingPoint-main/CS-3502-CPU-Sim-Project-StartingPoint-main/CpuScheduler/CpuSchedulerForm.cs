...
namespace CpuScheduler
{
    ...
    public partial class CpuSchedulerForm : Form
    {
        ...
        private readonly int[] mlfqTimeQuanta = new int[] { 2, 4, 8 };
        private const int MLFQ_PRIORITY_BOOST_INTERVAL = 20;
        ...
        public CpuSchedulerForm()
        {
            ...
            InitializeCustomAlgorithmButtons();
        }
        ...
        private void InitializeCustomAlgorithmButtons()
        {
            btnSRTF = new Button
            {
                Enabled = true,
            ...
            btnSRTF.FlatAppearance.MouseOverBackColor = Color.PaleGreen;
            btnSRTF.Click += new EventHandler(this.SRTFButton_Click);
            ...
            btnMLFQ.FlatAppearance.MouseOverBackColor = Color.PaleGreen;
            btnMLFQ.Click += new EventHandler(this.MLFQButton_Click);

            algorithmButtonPanel.Controls.Add(btnSRTF);
            algorithmButtonPanel.Controls.Add(btnMLFQ);
        }
...
4. Compare algorithm behaviors and outcomes";
        ...
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
        {
            ...
            return processResults.Values.OrderBy(r => r.StartTime).ThenBy(r => r.ProcessID).ToList();
        }
        ...
        private List<SchedulingResult> RunMLFQAlgorithm(List<ProcessData> processes)
        {
            var processResults = new Dictionary<string, SchedulingResult>();
            var remainingBurstTimes = new Dictionary<string, int>();
            var usedTimeInCurrentQueue = new Dictionary<string, int>();
            var currentQueueLevel = new Dictionary<string, int>();
            var queues = new Queue<ProcessData>[mlfqTimeQuanta.Length];

            for (int i = 0; i < queues.Length; i++)
            {
                queues[i] = new Queue<ProcessData>();
            }

            foreach (var process in processes)
            {
                remainingBurstTimes[process.ProcessID] = process.BurstTime;
                usedTimeInCurrentQueue[process.ProcessID] = 0;
                currentQueueLevel[process.ProcessID] = 0;
                processResults[process.ProcessID] = new SchedulingResult
                {
                    ProcessID = process.ProcessID,
                    ArrivalTime = process.ArrivalTime,
            ...
            var sortedByArrival = processes.OrderBy(p => p.ArrivalTime).ThenBy(p => p.ProcessID).ToList();
            var nextArrivalIndex = 0;
            var currentTime = 0;
            var completedProcesses = 0;
            ProcessData runningProcess = null;
            var runningQueueLevel = 0;

            while (completedProcesses < processes.Count)
            {
                while (nextArrivalIndex < sortedByArrival.Count && sortedByArrival[nextArrivalIndex].ArrivalTime <= currentTime)
                {
                    var arrivingProcess = sortedByArrival[nextArrivalIndex];
                    currentQueueLevel[arrivingProcess.ProcessID] = 0;
                    usedTimeInCurrentQueue[arrivingProcess.ProcessID] = 0;
                    queues[0].Enqueue(arrivingProcess);
                    nextArrivalIndex++;
                }

                if (currentTime > 0 && currentTime % MLFQ_PRIORITY_BOOST_INTERVAL == 0)
                {
                    if (runningProcess != null)
                    {
                        queues[0].Enqueue(runningProcess);
                        runningProcess = null;
                    }

                    for (int level = 1; level < queues.Length; level++)
                    {
                        while (queues[level].Count > 0)
                        {
                            var boostedProcess = queues[level].Dequeue();
                            queues[0].Enqueue(boostedProcess);
                        }
                    }
                ...
                if (runningProcess != null && runningQueueLevel > 0)
                {
                    var higherPriorityWorkAvailable = false;
                    for (int level = 0; level < runningQueueLevel; level++)
                    {
                        if (queues[level].Count > 0)
                        {
                            higherPriorityWorkAvailable = true;
                            break;
                        }
                    }

                    if (higherPriorityWorkAvailable)
                    {
                        queues[runningQueueLevel].Enqueue(runningProcess);
                        runningProcess = null;
                    }
                }
                ...
                if (runningProcess == null)
                {
                    for (int level = 0; level < queues.Length; level++)
                    {
                        if (queues[level].Count > 0)
                        {
                            runningProcess = queues[level].Dequeue();
                            runningQueueLevel = level;
                            break;
                        }
                    }
                }
                ...
                var runningId = runningProcess.ProcessID;
                var result = processResults[runningId];
                ...
                remainingBurstTimes[runningId]--;
                usedTimeInCurrentQueue[runningId]++;
                currentTime++;

                if (remainingBurstTimes[runningId] == 0)
                {
                    result.FinishTime = currentTime;
                    result.TurnaroundTime = result.FinishTime - result.ArrivalTime;
                    result.WaitingTime = result.TurnaroundTime - result.BurstTime;
                    completedProcesses++;
                    usedTimeInCurrentQueue[runningId] = 0;
                    runningProcess = null;
                    continue;
                }

                if (usedTimeInCurrentQueue[runningId] >= mlfqTimeQuanta[runningQueueLevel])
                {
                    usedTimeInCurrentQueue[runningId] = 0;

                    if (runningQueueLevel < queues.Length - 1)
                    {
                        runningQueueLevel++;
                        currentQueueLevel[runningId] = runningQueueLevel;
                    }
        ...
        private PerformanceMetrics CalculatePerformanceMetrics(List<SchedulingResult> results)
        {
            ...
            metrics.AverageWaitingTime = results.Average(r => r.WaitingTime);
            metrics.AverageTurnaroundTime = results.Average(r => r.TurnaroundTime);
            metrics.AverageResponseTime = results.Average(r => r.StartTime - r.ArrivalTime);
            ...
            var totalElapsedTime = latestFinish - earliestArrival;

            if (totalElapsedTime > 0)
            {
                metrics.CpuUtilization = ((double)totalBurstTime / totalElapsedTime) * 100.0;
                metrics.Throughput = (double)results.Count / totalElapsedTime;
            }
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            summaryItem.SubItems.Add($"Avg Wait: {metrics.AverageWaitingTime:F2}");
            summaryItem.SubItems.Add($"Avg Turn: {metrics.AverageTurnaroundTime:F2}");
            summaryItem.SubItems.Add($"CPU Util: {metrics.CpuUtilization:F2}%");
            summaryItem.SubItems.Add($"Throughput: {metrics.Throughput:F2}");
            listView1.Items.Add(summaryItem);

            var responseItem = new ListViewItem("METRIC");
            responseItem.SubItems.Add("Avg Response Time");
            responseItem.SubItems.Add($"{metrics.AverageResponseTime:F2}");
        ...
        private void MLFQButton_Click(object sender, EventArgs e)
        {
            ...
            if (processData.Count > 0)
            {
                ...
                DisplaySchedulingResults(results,
                    $"MLFQ (queues={mlfqTimeQuanta.Length}, quanta={string.Join("/", mlfqTimeQuanta)}, boost={MLFQ_PRIORITY_BOOST_INTERVAL})");
        ...
        private void ApplyLightTheme()
        {
            ...
            btnSRTF.BackColor = Color.Wheat;
            btnMLFQ.BackColor = Color.LemonChiffon;
